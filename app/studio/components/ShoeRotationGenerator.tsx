...
import { Copy, Save, Eye, Footprints } from "lucide-react";
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[minmax(320px,420px)_minmax(0,1fr)_minmax(300px,380px)] gap-6 w-full font-sans">
      ...
      <div className="flex flex-col gap-4 w-full min-w-0">
        <div className="flex items-center gap-2 px-1">
          <Footprints className="w-3.5 h-3.5" style={{ color: activeAccent.hex }} />
          <h2 className="text-[11px] font-bold uppercase tracking-widest text-[#f2f4f7] font-mono">SHOE DATA</h2>
        </div>
        ...
        <div className="bg-surface border border-brand-border p-5 rounded-xl flex flex-col gap-4 shadow-xl">
          ...
          <button 
            onClick={handleCopy} 
            className="w-full mt-2 py-2.5 bg-transparent border rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-[0.98] hover:bg-gray-800"
            style={{ borderColor: activeAccent.hex, color: activeAccent.hex }}
          ><Copy className="w-3.5 h-3.5" style={{ color: activeAccent.hex }} /> COPY SHOE LOG</button>
        </div>
      </div>
        ...
        <div ref={containerRef} className="w-full bg-[radial-gradient(#22252a_1px,transparent_1px)] [background-size:16px_16px] bg-[#07080a] border border-brand-border rounded-xl p-4 md:p-8 flex items-center justify-center flex-1 min-h-[500px] xl:min-h-[600px] relative shadow-inner overflow-clip">
          ...
          >
            <div
              ...
              style={{ 
                minHeight: '440px',
                borderColor: template === 'rotation board' ? `${activeAccent.hex}30` : undefined,
                borderTopColor: template === 'rotation board' ? activeAccent.hex : undefined
              }}
            >
             ...
             {template === 'rotation board' && (
               <>
                 <div className="flex justify-between items-start mb-6">
                   ...
                   <div className="bg-[#22252a] px-3 py-1 text-[10px] font-bold uppercase tracking-widest rounded-sm" style={{ color: activeAccent.hex }}>{formData.primaryUse}</div>
                 </div>
                   ...
                   {formData.maxDistance && (
                     <div className="w-full">
                       ...
                       <div className="w-full h-1.5 bg-[#22252a] rounded-full overflow-hidden">
                         <div className="h-full" style={{ width: usagePercent === '—' ? '0%' : usagePercent, backgroundColor: activeAccent.hex }}></div></div></div>)}
</div>
             ...
             {template === 'shoe log' && (
               <>
                 ...
                 {formData.lastNote && (
                   <div className="mt-auto p-3 bg-[#111113] border-l-2 text-xs text-gray-400" style={{ borderColor: activeAccent.hex }}>
                     {formData.lastNote}
                               </div>
            )}
