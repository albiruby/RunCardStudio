...
export default function PaceBandGenerator({ previewRef, showToast }: PaceBandProps) {
  ...
  const safeTemplate = ['wristband', 'phone-lockscreen', 'printable-a4'].includes(template) ? template : 'wristband';
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        // Base intrinsic size on template when specific templates are selected
        if (safeTemplate === 'phone-lockscreen') { targetW = 360; targetH = 640; }
        else if (safeTemplate === 'printable-a4') { targetW = 595; targetH = 842; }
        else if (safeTemplate === 'wristband') { targetW = 150; targetH = 450; }
        else {
        ...
        // We leave 48px horizontal padding, and 100px vertical padding
  ...
  // Validation
  const hrVal = formData.hr ? parseInt(formData.hr, 10) : 0;
  const minVal = formData.min ? parseInt(formData.min, 10) : 0;
  const secVal = formData.sec ? parseInt(formData.sec, 10) : 0;
        ...
        <div ref={containerRef} className="w-full bg-[radial-gradient(#22252a_1px,transparent_1px)] [background-size:16px_16px] bg-[#07080a] border border-brand-border rounded-xl p-4 md:p-8 flex items-center justify-center flex-1 min-h-[500px] xl:min-h-[600px] relative shadow-inner overflow-clip">
          ...
          >
            <div ref={previewRef} className={`relative flex items-center justify-center shadow-[0_20px_50px_rgba(0,0,0,0.8)] overflow-hidden transition-all duration-300 select-none
               ${safeTemplate === 'wristband' ? 'w-[150px] bg-white text-black min-h-[450px] border border-gray-300 rounded' : ''}
               ${safeTemplate === 'phone-lockscreen' ? 'w-[360px] h-[640px] bg-[#0a0b0d] text-white border-4 border-[#22252a] rounded-[2.5rem] overflow-hidden' : ''}
               ${safeTemplate === 'printable-a4' ? 'w-[595px] min-h-[842px] bg-white text-black border border-gray-300' : ''}
            `}>
              
              {(() => {
                const isInputMissing = dist <= 0 || rawSecs <= 0 || isTimeFormatInvalid;
                const isDarkTemplate = safeTemplate === 'phone-lockscreen';
                ...
                return (
                  <>
                    ...
                    <div className={`absolute bottom-3 left-0 right-0 text-center font-mono text-[9px] tracking-[0.25em] uppercase opacity-40 ${safeTemplate === 'phone-lockscreen' ? 'text-gray-400' : 'text-gray-500'}`}>
