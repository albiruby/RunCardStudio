...
const file = 'app/studio/components/RaceSplitGenerator.tsx';
let content = fs.readFileSync(file, 'utf8');

const strToMove = `  const getPlainFormDataForCurrentCard = () => {
    return { ...formData };
  };
...
let newContent = content.replace(strToMove, '');
/* 
Wait, the string match might be exact but line breaks might cause issues. 
I'll just inject it right before `return (` which is around line 226 in newContent.
*/
...
if (idx !== -1) {
   lines.splice(idx, 0, strToMove);
} else {
   console.log("Could not find line '  return ('");
}

fs.writeFileSync(file, lines.join('\n'));
console.log('Fixed RaceSplitGenerator manually');
